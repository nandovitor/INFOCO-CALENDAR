...
const Calendar: React.FC<CalendarProps> = ({ events, title }) => {
    ...
    daysGrid.push(
      <div key={day} className="p-1 flex justify-center items-center">
        <div
          title={tooltipText}
          className={cn(
            "w-9 h-9 flex items-center justify-center rounded-full text-sm transition-colors duration-200",
            hasEvent ? eventColorClass : "text-gray-700 hover:bg-gray-100",
  ...
  return (
    <Card className="flex flex-col h-full">
      ...
      <div className="flex justify-between items-center px-4 py-2 mt-2">
        <button onClick={() => changeMonth(-1)} className="p-2 rounded-full hover:bg-gray-100 text-gray-600" aria-label="Previous month">
          <ChevronLeft size={20} />
        </button>
        <h4 className="font-semibold text-gray-700 capitalize w-32 text-center">{`${monthName} ${year}`}</h4>
        <button onClick={() => changeMonth(1)} className="p-2 rounded-full hover:bg-gray-100 text-gray-600" aria-label="Next month">
